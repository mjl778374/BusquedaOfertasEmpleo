...
try
{
    ...
    $IdAliasExcluir = CParametrosGet::ValidarIdEntero("IdAliasExcluir", $NumError);
    if ($NumError == 1)
        throw new Exception("Debe incorporar el parámetro 'IdAliasExcluir'.");
    elseif ($NumError == 2)
        throw new Exception("'IdAliasExcluir' debe ser un número entero mayor o igual que 0.");
    elseif ($NumError != 0)
        throw new Exception("No se manejó el error número " . $NumError . " en el parámetro 'IdAliasExcluir'.");
    ...
    if (strcmp($Modo, $MODO_CAMBIO) == 0)
    {
        ...
        if (strcmp($_GET["IdProfesionOficio"], $IdProfesionOficio) != 0 || strcmp($_GET["IdAlias"], $IdAlias) != 0  || strcmp($_GET["IdAliasExcluir"], $IdAliasExcluir) != 0)
            header("Location: " . "aliasProfesionOficio.php?Modo=" . $Modo . "&IdProfesionOficio=" . $IdProfesionOficio . "&IdAlias=" . $IdAlias  . "&IdAliasExcluir=" . $IdAliasExcluir);
    } // if (strcmp($Modo, $MODO_CAMBIO) == 0)

    elseif (strcmp($_GET["IdProfesionOficio"], $IdProfesionOficio) != 0 || strcmp($_GET["IdAliasExcluir"], $IdAliasExcluir) != 0)
        header("Location: " . "aliasProfesionOficio.php?Modo=" . $Modo . "&IdProfesionOficio=" . $IdProfesionOficio . "&IdAliasExcluir=" . $IdAliasExcluir);
} // try
catch (Exception $e)
{
    $NumError = 1;
...
try
{
    ...
    if ($NumError == 0 && $SePretendeGuardarInformacion)
    {
        ...
        if (strcmp($Modo, $MODO_ALTA) == 0)
            $AliasProfesionesOficios->AltaAliasProfesionOficio($IdProfesionOficio, $Alias, $NumError, $ObjAliasProfesionOficio);
...
if ($SeGuardoInformacionExitosamente && (strcmp($Modo, $MODO_ALTA) == 0 || strcmp($Modo, $MODO_CAMBIO) == 0 && $IdAliasOriginal != $IdNuevoAlias))
    header("Location: " . "aliasProfesionOficio.php?Modo=" . $MODO_CAMBIO . "&IdProfesionOficio=" . $IdProfesionOficio . "&IdAlias=" . $IdNuevoAlias  . "&IdAliasExcluir=" . $IdAliasExcluir); // Se carga el alias de profesión u oficio guardado.
...
elseif ($SeGuardoInformacionExitosamente)
    $MensajeXDesglosar = CFormateadorMensajes::FormatearMensajeOK("Se guardó el alias exitosamente.");
    ...
    <div class="container mt-4">
        <div class="form-row justify-content-center">
            <div class="form-group col-8 col-md-6 col-lg-4">
                <label for="ProfesionOficio">Alias</label>
                <input type="text" class="form-control" id="Alias" name="Alias" placeholder="Ingrese el alias" value="<?php echo $Alias; ?>" maxlength="<?php echo $MaximoTamanoCampoAlias;?>">
            </div>
        </div>
        <div class="form-row justify-content-center mt-4">
            <div class="form-group col-8 col-md-6 col-lg-4">
                <button type="submit" class="btn btn-primary">Guardar</button>
                <button type="button" class="btn btn-primary" onclick="window.location.href='todosAliasDeProfesionOficio.php?Modo=<?php echo $MODO_CAMBIO?>&IdProfesionOficio=<?php echo $IdProfesionOficio?>&IdAlias=<?php echo $IdAliasExcluir?>';">Regresar</button>
            </div>
        </div>
    </div>
<?php
